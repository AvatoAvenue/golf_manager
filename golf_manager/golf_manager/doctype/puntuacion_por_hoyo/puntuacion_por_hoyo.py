...
class puntuacionporhoyo(Document):
    ...
    def on_update(self):
        # Si estamos en una carga masiva no hacer nada
        if getattr(frappe.local, "bulk_puntuacion_update", False):
            return
        # Si el documento tiene el flag específico, tampoco
        if getattr(self.flags, "skip_total_update", False):
            return
