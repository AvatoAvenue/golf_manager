...
def guardar_fila_puntuaciones(ronda, jugador, hoyos_json):
    ...
    # Activar modo bulk para evitar recálculos automáticos durante la carga masiva
    frappe.local.bulk_puntuacion_update = True
    ...
    try:
        ...
        for hoyo in hoyos:
            ...
            # Flag adicional por si acaso
            doc.flags.skip_total_update = True
            doc.save()
            guardados.append(numero)

        # Recalcular UNA SOLA VEZ al terminar todos los hoyos
        if guardados:
            ...
            if torneo:
                ...
                if participacion:
                    # Reintentar en caso de SerializationFailure
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            total = _calcular_total_jugador(torneo, jugador)
                            frappe.db.set_value(
                                "participacion en torneo",
                                participacion,
                                "puntuacion_total_acumulada",
        ...
        # Desactivar el modo bulk
        frappe.local.bulk_puntuacion_update = False
